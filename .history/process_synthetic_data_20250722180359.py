...
# Since the data is 72 months for 5 cities, we structure it that way.

# Create one 72-month date range (6 years)
date_range_single_city = pd.date_range(start='2017-01-01', periods=72, freq='M')

# Get the unique city names from the newly created 'city' column
cities = df_final['city'].unique()

# Create a new DataFrame by repeating the date range for each city
structured_dfs = []
...
# Combine the new date/city columns with your synthetic data
# This assumes df_final is already ordered by city (which it should be)
df_final.reset_index(drop=True, inplace=True)
final_index_df.reset_index(drop=True, inplace=True)

df_final_structured = pd.concat([final_index_df, df_final], axis=1)
